package com.example.movetasker.service
...
import com.example.movetasker.R
...
class FileWatchService : Service() {
    ...
    private lateinit var repository: MoveTaskerRepository
    ...
    private fun handleFile(rule: RuleEntity, file: File) {
        serviceScope.launch {
            if (repository.hasLogForSource(file.absolutePath)) return@launch
    ...
    ) : FileObserver(rule.sourcePath, CLOSE_WRITE) {
        override fun onEvent(event: Int, path: String?) {
            if (event != CLOSE_WRITE || path.isNullOrBlank()) return
